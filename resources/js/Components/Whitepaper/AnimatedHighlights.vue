<template>
    <section class="section bg-slate-50 relative overflow-hidden">
        ...
        <div class="absolute inset-0 pointer-events-none" aria-hidden="true">
            ...
            <!-- Floating hexagons -->
            <svg class="absolute top-10 right-10 w-20 h-20 text-prex-200/30 animate-rotate-slow" viewBox="0 0 100 100"><polygon points="50,5 93,25 93,75 50,95 7,75 7,25" fill="none" stroke="currentColor" stroke-width="2" /></svg>
            <svg class="absolute bottom-20 left-10 w-16 h-16 text-pusd-300/25 animate-rotate-slower" style="animation-direction: reverse" viewBox="0 0 100 100"><polygon points="50,5 93,25 93,75 50,95 7,75 7,25" fill="none" stroke="currentColor" stroke-width="2" /></svg>
                ...
                <div v-for="(highlight, index) in keyHighlights" :key="highlight.title" :class="['group relative bg-white border border-slate-200 rounded-2xl p-6 shadow-sm transition-all duration-500 hover:shadow-xl hover:border-slate-300 hover:-translate-y-1', gridVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-10 scale-95']" :style="{ transitionDelay: `${index * 100 + 100}ms` }">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative w-14 h-14 mb-5">
                            ...
                            <div class="relative w-full h-full rounded-xl bg-prex-50 border border-prex-200 flex items-center justify-center group-hover:border-transparent group-hover:bg-gradient-to-br group-hover:from-slate-50 group-hover:to-white transition-all duration-300">
                                <svg class="w-7 h-7 text-prex-600 group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="highlight.iconPath" /></svg>
                            </div>
                        </div>

                        <h3 class="text-lg font-semibold text-slate-900 mb-2 group-hover:text-prex-700 transition-colors">{{ highlight.title }}</h3>
...
const keyHighlights = [
    { iconPath: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z', title: 'Treasury-Backed', description: '50% of all protocol revenue flows into the Solvency Pool that backs wUSD redemptions.', gradient: 'from-prex-400 to-blue-500', bgGlow: 'bg-prex-400/10' },
    ...
    { iconPath: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', title: '3-Month Cliff', description: 'Cliff period prevents short-term extraction and aligns incentives with long-term platform growth.', gradient: 'from-pusd-400 to-amber-500', bgGlow: 'bg-pusd-400/10' },
